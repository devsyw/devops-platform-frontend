...
import { getCustomer, deleteCustomer, activateCustomer } from '../../api/customerApi';
import { getProjects, createProject, deleteProject, activateProject } from '../../api/projectApi';
import { FiEdit, FiArrowLeft, FiTrash2, FiPlus, FiFolder, FiX, FiRefreshCw } from 'react-icons/fi';
...
const CustomerDetailPage = () => {
  ...
  const fetchData = () => {
    Promise.all([
      getCustomer(id),
      getProjects(id, { includeInactive: true })
    ]).then(([cRes, pRes]) => {
  ...
  const handleDeleteCustomer = async () => {
    ...
    try { await deleteCustomer(id); toast.success('비활성화 완료'); fetchData(); }
    catch { toast.error('처리 실패'); }
  };

  const handleActivateCustomer = async () => {
    if (!window.confirm('이 고객사를 다시 활성화하시겠습니까?')) return;
    try { await activateCustomer(id); toast.success('활성화 완료'); fetchData(); }
    catch { toast.error('처리 실패'); }
  };
  ...
  const handleActivateProject = async (projId, projName) => {
    if (!window.confirm(projName + ' 프로젝트를 활성화하시겠습니까?')) return;
    try { await activateProject(id, projId); toast.success('활성화 완료'); fetchData(); }
    catch { toast.error('처리 실패'); }
  };
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {customer.isActive ? (
            ...
            <button className="btn btn--primary" onClick={handleActivateCustomer}>활성화</button>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        {!customer.isActive && (
          <div style={{
            gridColumn: '1 / -1', padding: '12px 16px', borderRadius: 8,
            background: '#fff3cd', border: '1px solid #ffeaa7', display: 'flex',
            justifyContent: 'space-between', alignItems: 'center'
          }}>
            <span style={{ fontSize: 13, color: '#856404' }}>⚠️ 이 고객사는 현재 <strong>비활성</strong> 상태입니다.</span>
            <button className="btn btn--primary btn--sm" onClick={handleActivateCustomer}>활성화</button>
          </div>
        )}
        <div className="card">
      ...
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <h3>프로젝트 ({projects.filter(p => p.isActive).length})</h3>
        ...
        {projects.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {projects.map(p => (
              <div key={p.id} style={{
                border: '1px solid ' + (p.isActive ? '#e9ecef' : '#f5c6cb'),
                borderRadius: 8, padding: 16,
                opacity: p.isActive ? 1 : 0.6,
                background: p.isActive ? '#fff' : '#fdf2f2'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <FiFolder color={p.isActive ? '#0984e3' : '#adb5bd'} />
                    <strong>{p.name}</strong>
                    {p.code && <span style={{ fontSize: 12, color: '#808e9b' }}>({p.code})</span>}
                    {!p.isActive && <span className="status-badge status-badge--expired" style={{ fontSize: 10 }}>비활성</span>}
                  </div>
                  {p.isActive ? (
                    <button className="btn btn--sm btn--ghost" onClick={() => handleDeleteProject(p.id, p.name)} title="비활성화">
                      <FiTrash2 size={13} color="#e74c3c" />
                    </button>
                  ) : (
                    <button className="btn btn--sm btn--ghost" onClick={() => handleActivateProject(p.id, p.name)} title="활성화">
                      <FiRefreshCw size={13} color="#0984e3" />
                    </button>
                  )}
                </div>
